...
HT::HTHANDLE* HTCOM_STATIC::Create(OS13HANDLE h, int Capacity, int SecSnapshotInterval, int MaxKeyLength, int MaxPayloadLength, const char FileName[512])
{
	try {
		HT::HTHANDLE* result;
		...
		if (SUCCEEDED(hr0))
		{
			HRESULT hr1 = pIHTCreate->HTCreate(Capacity, SecSnapshotInterval, MaxKeyLength, MaxPayloadLength, FileName, result);
			if (SUCCEEDED(hr1))
			{
				pIHTCreate->Release();
				return result;
