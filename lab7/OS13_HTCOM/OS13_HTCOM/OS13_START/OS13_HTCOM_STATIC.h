...
namespace HTCOM_STATIC
{
	...
	HT::HTHANDLE* Create(OS13HANDLE h, int Capacity, int SecSnapshotInterval, int MaxKeyLength, int MaxPayloadLength, const char HTUsersGroup[512], const char FileName[512]);

	HT::HTHANDLE* Start(OS13HANDLE h, const char FileName[512]);

	HT::HTHANDLE* Start2(OS13HANDLE h, const char HTUser[512], const char HTPassword[512], const char FileName[512]);
...
interface IHTStart : IUnknown
{
	...
	virtual HRESULT __stdcall HTOpen
	(
		const char HTUser[512],
		const char HTPassword[512],
		const char FileName[512],
		HT::HTHANDLE*& htHandle
	) = 0;
};
