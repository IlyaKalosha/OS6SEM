...
HRESULT  STDMETHODCALLTYPE OS13_HTCOM::HTCreate(int Capacity, int SecSnapshotInterval, int MaxKeyLength, int MaxPayloadLength, const char HTUsersGroup[512], const char FileName[512], HT::HTHANDLE*& htHandle)
{
	...
	try {
		...
		BOOL cancreate = 0;
		wchar_t group[512];
		mbstowcs(group, HTUsersGroup, 512);
		if (GetUserName(un, &lenun))
		{
			...
			if (ns == NERR_Success)
			{
				for (int i = 0; i < uc; i++)
				{
					if (!wcscmp(buf[i].grui0_name, group) || !wcscmp(buf[i].grui0_name, L"Администраторы"))
						cancreate = 1;
				}
			}
		}
		if (cancreate) {
			HT::HTHANDLE*	NewHT = new HT::HTHANDLE(Capacity, SecSnapshotInterval, MaxKeyLength, MaxPayloadLength, HTUsersGroup, FileName);
...
HRESULT  STDMETHODCALLTYPE OS13_HTCOM::HTOpen(const char FileName[512], HT::HTHANDLE*& htHandle)
{
	...
	try {
		...
		HT::HTHANDLE* NewHT = new HT::HTHANDLE(GetHT.Capacity, GetHT.SecSnapshotInterval, GetHT.MaxKeyLength, GetHT.MaxPayloadLength, GetHT.HTUsersGroup, FileName);
		NewHT->File = hf;
		mbstowcs(group, NewHT->HTUsersGroup, 512);
		if (GetUserName(un, &lenun))
		{
			NET_API_STATUS ns = NetUserGetLocalGroups
			(
...
HRESULT  STDMETHODCALLTYPE OS13_HTCOM::HTOpen(const char HTUser[512], const char HTPassword[512], const char FileName[512], HT::HTHANDLE*& htHandle)
{
	SECURITY_ATTRIBUTES attributes;
	ZeroMemory(&attributes, sizeof(attributes));
	attributes.nLength = sizeof(attributes);
	...
	try {
		...
		mbstowcs(group, NewHT->HTUsersGroup, 512);
		mbstowcs(user, HTUser, 512);
		mbstowcs(password, HTPassword, 512);
		...
		BOOL reslog = LogonUser(user, L".", password, LOGON32_LOGON_INTERACTIVE, LOGON32_PROVIDER_DEFAULT, &hToken);
		CloseHandle(hToken);
		
		if (reslog)
			passwordIsValid = 1;

		if (cancreate==1 && passwordIsValid==1) {
			HANDLE hm = CreateFileMapping(
				hf, &attributes,
				PAGE_READWRITE,
				0,
